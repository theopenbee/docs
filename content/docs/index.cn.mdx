---
title: 简介
description: OpenBee — 将 Agent 作为你的数字员工运行
---

OpenBee 是一个数字员工解决方案，将 Agent 作为自主数字员工运行。每个数字员工都能够进行多步骤任务规划和独立执行，通过你现有的即时通讯平台进行沟通。

## 核心特性

- **AI 数字员工** — 具有持久记忆和 MCP 工具调用能力的 Agent 代理
...
- **中枢大脑（Agent）**：系统核心，负责理解消息意图。简单请求可直接回复 IM 平台；复杂任务则生成任务指令，交由任务分配环节处理。
- **任务分配**：将具体任务下发给合适的数字员工执行。
- **数字员工**：独立的 Agent 代理，接收任务后进行多步骤规划和自主执行，完成后将结果回传给 IM 平台。
