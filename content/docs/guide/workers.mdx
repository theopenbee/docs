...
Workers are AI agents that execute tasks on your behalf. Each worker has its own name, description, constraints, and working directory. You can manage workers through the web console or by chatting directly with the Bee (coordinator) in your IM platform.
...
The Bee will create the worker and confirm with you. You can specify a name, description, and initial constraints in your message.

### Update a Worker

> "Update Xiaoming's description to focus on Go code reviews"

> "Change DocWriter's constraints to always write in Chinese"
...
| **Constraints** | Persistent rules that shape the worker's behavior |
...
Constraints are written to the worker's system prompt and persist across all tasks. Use them to define the worker's expertise, communication style, and rules.
...
You can update constraints at any time through conversation:

> "Update Xiaoming's constraints to include: always run tests before committing"
...
- **Response quality** — focused constraints produce more relevant output
...
Each worker has a `CLAUDE.md` file in its working directory that defines the worker's system prompt. This file is automatically managed by OpenBee based on the worker's constraints field, but you can also edit it directly for advanced customization.
